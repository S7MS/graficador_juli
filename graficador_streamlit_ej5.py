...
if ejercicio == "Ejercicio 1: Andrés y Beatriz":
    ...
    st.markdown("""
    Se analiza la frontera de posibilidades de utilidad (FPU) entre Andrés y Beatriz, dadas sus funciones de utilidad y una restricción de producción.
    """)
    
    st.sidebar.header("Parámetros del Ejercicio 1")
    total_income = st.sidebar.slider("Ingreso Total (x+y)", 0.5, 5.0, 1.0, 0.1)

    st.latex(f"u(y) = 0.4y")
    st.latex(f"v(x,y) = 0.4x + [0.2 + 0.8y – y^2]")
    st.latex(f"x + y = {total_income}")
    ...
    with col1:
        ...
        # Cálculos
        y = np.linspace(0, total_income, 200)
        x = total_income - y
        u = 0.4 * y
        v = 0.4 * x + (0.2 + 0.8 * y - y**2)

        # Óptimo para Andrés: maximiza u(y), lo que ocurre cuando 'y' es máximo
        y_andres_opt = total_income
        x_andres_opt = 0.0
        u_andres_opt = 0.4 * y_andres_opt
        v_andres_opt = 0.4 * x_andres_opt + (0.2 + 0.8 * y_andres_opt - y_andres_opt**2)

        # Óptimo para Beatriz: maximiza v(x,y) = v(total_income-y, y) = 0.4(total_income-y) + 0.2 + 0.8y - y^2
        # Derivando respecto a y: 0.4 - 2y = 0 => y = 0.2
        y_beatriz_opt = 0.2
        # Asegurarse que el óptimo no exceda el ingreso total
        if y_beatriz_opt > total_income:
            y_beatriz_opt = total_income
        
        x_beatriz_opt = total_income - y_beatriz_opt
        u_beatriz_opt = 0.4 * y_beatriz_opt
        v_beatriz_opt = 0.4 * x_beatriz_opt + (0.2 + 0.8 * y_beatriz_opt - y_beatriz_opt**2)

        st.markdown(f"""
        - **Distribución óptima para Andrés**: Se logra cuando su ingreso `y` es máximo (`y={total_income}`, `x=0`).
          - Utilidad de Andrés: $u = {u_andres_opt:.2f}$
          - Utilidad de Beatriz: $v = {v_andres_opt:.2f}$
        - **Distribución óptima para Beatriz**: Se logra maximizando su utilidad (`y=0.2`, si es posible).
          - Con `y={y_beatriz_opt:.2f}`, `x={x_beatriz_opt:.2f}`.
          - Utilidad de Andrés: $u = {u_beatriz_opt:.2f}$
          - Utilidad de Beatriz: $v = {v_beatriz_opt:.2f}$
        """)
    ...
    with col2:
        ...
        ax.scatter(u_andres_opt, v_andres_opt, color='blue', zorder=5, label=f'Óptimo para Andrés (y={y_andres_opt:.1f})')
        ax.scatter(u_beatriz_opt, v_beatriz_opt, color='red', zorder=5, label=f'Óptimo para Beatriz (y={y_beatriz_opt:.1f})')
...
# --- EJERCICIO 2 ---
    ...
    """)
    st.sidebar.header("Parámetros del Ejercicio 2")
    alpha = st.sidebar.slider("Parámetro de ponderación (α)", 0.0, 2.0, 2/3, 0.01)

    col1, col2 = st.columns([2, 3])

    # FPU del ejercicio 1 (asumiendo ingreso total = 1 para mantener la forma de la función)
...
# --- EJERCICIO 3 ---
    ...
    """)
    st.sidebar.header("Parámetros del Ejercicio 3")
    x_income = st.sidebar.slider("Ingreso de X (más rico)", 1.0, 200.0, 50.0, 0.5)
    ...
    with col1:
        ...
        st.info("El bienestar es más sensible a los aumentos de renta de la persona más pobre.")
        ...
        st.info("La redistribución del rico al pobre aumenta el bienestar.")
...
# --- EJERCICIO 4 ---
    ...
    """)
    st.sidebar.header("Parámetros del Ejercicio 4")
    y_medina_total = st.sidebar.slider("Ingreso Total del Matrimonio", 100, 2000, 840, 10)
    y_pablo = st.sidebar.slider("Ingreso de Pablo (necesitado)", 10, 200, 60, 5)
    ...
    with col1:
        ...
        TM = max(0, TM) # La transferencia no puede ser negativa
    ...
    with col2:
        ...
        y_alba = y_medina_total / 2
        y_benjamin = y_medina_total / 2
        ...
        t = max(0, t) # La transferencia no puede ser negativa
        tA = t
        tB = t
        TD = tA + tB
        cP_b = y_pablo + TD
        st.metric("Transferencia total de divorciados (TD)", f"{TD:.2f} €")
        st.markdown(f"(tA = {tA:.2f}€, tB = {tB:.2f}€)")
    ...
    st.warning("""
    **Interpretación (Problema del Free-Rider)**: Cuando deciden por separado, tanto Alba como Benjamín tienen un incentivo a contribuir menos, esperando que el otro asuma parte de la "carga" de ayudar a Pablo. Cada uno no internaliza el beneficio completo que su contribución genera para el otro. Esto lleva a una provisión ineficientemente baja de la ayuda en comparación con la situación en la que actúan conjuntamente.
    """)
...
# --- EJERCICIO 5 ---
    ...
    """)
    st.sidebar.header("Parámetros del Ejercicio 5")
    coste_redistribucion = st.sidebar.slider("Coste de Redistribución", 1.0, 3.0, 2.0, 0.1)
    st.sidebar.info(f"Por cada 1€ que recibe Blanca, Nieves pierde {coste_redistribucion:.2f}€.")
    ...
    u_func = interp1d(renta_puntos, utilidad_puntos, kind='cubic', fill_value="extrapolate")
    ...
    # Por cada 1 que recibe Blanca (t), Nieves pierde 'coste_redistribucion'*t.
    t_max = 0.8 / coste_redistribucion 
    t_vals = np.linspace(0, t_max, 100)
    c_blanca_con = 0.2 + t_vals
    c_nieves_con = 0.8 - coste_redistribucion * t_vals
    # Asegurarse de que los consumos no sean negativos
    c_blanca_con = np.clip(c_blanca_con, 0, 1)
    c_nieves_con = np.clip(c_nieves_con, 0, 1)
    ...
    st.info("El coste de la redistribución 'encoge' la frontera de posibilidades de utilidad. Esto puede llevar a que la política redistributiva óptima sea menos ambiciosa, ya que parte del bienestar se 'pierde' en el proceso.")
